...
function image(model, serializer) {
  ...
  model
    ...
    .action('find', find)
      ...
      .param('imageId', 'array').format('basic', 'ImageId') // ImageId.n
        // .to(array('ImageId'))
      ...
      // .validate
      .param('description', 'text').format('filter')
      // how to handle this, since this is both a query param and a filter?
      // .param('image-id')
      .param('image-type', 'string') // machine | kernel | ramdisk
      .param('public', 'boolean').format('filter', 'is-public')
      .param('kernel-id', 'string').format('filter')
      .param('manifest-location').format('filter')
      ...
      .param('name').format('filter')
      .param('owner-alias').format('filter')
      .param('platform').format('filter')
      .param('product-code').format('filter')
      ...
      .param('status', 'string').format('filter', 'state') // available | pending | failed
