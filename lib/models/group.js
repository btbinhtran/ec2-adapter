...
function group(model) {
  model
    ...
    .attr('vpcId') // VpcId
    .action('find', find)
      .param('name', 'array') // GroupName.n
      .param('id', 'array') // GroupId.n
      .param('ownerId', 'string') // owner-id
    .action('create', create)
      // GroupName required, Accepts alphanumeric characters, spaces, periods, dashes, and underscores
      // GroupDescription required Accepts alphanumeric characters, spaces, periods dashes, and underscores
